...
test('session-start — compact 는 런타임 permission_mode 스탬프를 지우지 않는다 (H1-2)', () => {
  const d = pactRepo({ settingsYolo: false });
  try {
    fs.writeFileSync(path.join(d, '.pact', 'state.json'),
...
test('session-start — resume 는 새 프로세스 → stale yolo 스탬프 승계 안 함 (R3)', () => {
  const d = pactRepo({ settingsYolo: false });
  try {
    // 직전 세션의 yolo 런타임 스탬프. resume(플래그 없이 재개)는 이를 승계하면 안 됨.
    fs.writeFileSync(path.join(d, '.pact', 'state.json'),
      JSON.stringify({ permission_mode: 'bypassPermissions', is_yolo: true }));
    const r = runHook(SESSION_START, { session_id: 's1', cwd: d, hook_event_name: 'SessionStart', source: 'resume' }, d);
    assert.equal(r.status, 0, r.stderr);
    const st = readState(d);
    assert.notEqual(st.permission_mode, 'bypassPermissions', 'resume 는 stale 런타임 mode 를 리셋해야 함');
    assert.doesNotMatch(r.stdout, /yolo/, 'settings yolo 아니면 resume 에서 거짓 yolo 경고 금지');
  } finally { fs.rmSync(d, { recursive: true, force: true }); }
});

test('session-start — 손상 state 를 compact 에서 wipe 하지 않는다 (R3)', () => {
  const d = pactRepo({ settingsYolo: false });
  try {
    const sp = path.join(d, '.pact', 'state.json');
    fs.writeFileSync(sp, '{ torn ');
    const r = runHook(SESSION_START, { session_id: 's1', cwd: d, hook_event_name: 'SessionStart', source: 'compact' }, d);
    assert.equal(r.status, 0, r.stderr);
    assert.equal(fs.readFileSync(sp, 'utf8'), '{ torn ', '손상+compact 는 보존(다음 도구 호출이 재스탬프)');
  } finally { fs.rmSync(d, { recursive: true, force: true }); }
});

test('session-start — valid-JSON 스칼라 state 에도 크래시하지 않는다 (R3 typeof 가드)', () => {
  const d = pactRepo({ settingsYolo: false });
  try {
    fs.writeFileSync(path.join(d, '.pact', 'state.json'), '42');
    const r = runHook(SESSION_START, { session_id: 's1', cwd: d, hook_event_name: 'SessionStart', source: 'startup' }, d);
