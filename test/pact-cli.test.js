...
test('pact resume-prompt --consume — 카운트 영속 증가 (파일 기반 회로차단기)', () => {
  ...
  try {
    ...
    assert.equal(r1.escalate, false, 'RESUME1 은 유효 재투입');
    ...
    // RC-1: RESUME2 는 여전히 유효 재투입(escalate 아님) — 소비가 거부되지 않았으므로.
    assert.equal(r2.escalate, false, 'RESUME2 는 아직 유효 재투입(off-by-one 아님)');
    // 영속 파일 확인
...
test('pact resume-prompt --consume — 드라이버와 동일 예산: RESUME 2회 재투입 후 3번째에 escalate (RC-1)', () => {
  const repo = makeRepo();
  try {
    seedRun(repo, 'PACT-001');
    // 1·2번째 소비 = 유효 재투입(escalate 아님) — driver.mjs shouldResume(resumeCount<maxResume) 와 동형.
    const r1 = JSON.parse(runPact(['resume-prompt', 'PACT-001', '--consume'], repo).stdout);
    assert.equal(r1.escalate, false);
    assert.match(r1.continuationPrompt, /RESUME 1/);
    const r2 = JSON.parse(runPact(['resume-prompt', 'PACT-001', '--consume'], repo).stdout);
    assert.equal(r2.escalate, false, '2번째도 재투입(off-by-one 이면 여기서 조기 escalate)');
    assert.match(r2.continuationPrompt, /RESUME 2/);
    // 3번째 소비 = 소비 전 카운트가 이미 cap → 거부 → escalate. 카운트는 늘지 않음.
    const r3 = JSON.parse(runPact(['resume-prompt', 'PACT-001', '--consume'], repo).stdout);
    assert.equal(r3.escalate, true, '재개 상한 도달 후에만 escalate');
    assert.equal(r3.resumes_remaining, 0);
    const st = JSON.parse(fs.readFileSync(path.join(repo, '.pact/runs/PACT-001/resume.json'), 'utf8'));
    assert.equal(st.count, 2, 'cap 초과 소비는 카운트 불변');
  } finally { cleanupRepo(repo); }
});
