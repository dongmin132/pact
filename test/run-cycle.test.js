...
// ─── DRV-1: already_prepared(재개) 도 --graph 면 task_graph 재emit ──────────
// 크래시 재개/부분 사이클에서 prepare 가 already_prepared 로 단락되면 파이프라인이 아직
// admit 안 된 하위 DAG task 를 pull 대상으로 못 봐 조용히 누락(→ 잘못된 exit 0)한다.
// fresh prepare(--graph)와 동일하게 재개 경로도 task_graph 를 내보내야 한다.
test('DRV-1 prepare --graph — already_prepared 재개에도 task_graph 재emit (미-admit DAG 누락 방지)', () => {
  const dir = makeProject();
  try {
    writeTasks(dir, [
      { id: 'RESUME-001', allowed_paths: ['src/a.ts'] },
      { id: 'RESUME-002', allowed_paths: ['src/b.ts'] },
      { id: 'RESUME-003', allowed_paths: ['src/c.ts'], dependencies: ['RESUME-002'] },
    ]);
    // 1차: --max=1 → batch0 1개 + current_batch/worktree 생성 (이후 호출은 already_prepared)
    const r1 = JSON.parse(runPact(['run-cycle', 'prepare', '--max=1', '--graph'], dir).stdout);
    assert.equal(r1.already_prepared, undefined, '첫 호출은 정상 진행');
    assert.ok(r1.task_graph, '첫 호출도 --graph 면 task_graph 존재');
    // 2차: already_prepared 인데도 --graph 면 task_graph 가 다시 있어야 한다(핵심 결함).
    const r2 = JSON.parse(runPact(['run-cycle', 'prepare', '--max=1', '--graph'], dir).stdout);
    assert.equal(r2.already_prepared, true, '두 번째는 멱등 재개');
    assert.ok(r2.task_graph, 'DRV-1: 재개(already_prepared)에도 task_graph 재emit 돼야 함');
    const graphIds = Object.keys(r2.task_graph.tasks);
    assert.equal(graphIds.length, 2, `batch0 밖 하위 DAG 2개가 graph 에 담겨야: ${JSON.stringify(graphIds)}`);
    // batch0 task 는 graph 에서 제외 — current_batch 의 id 는 graph.tasks 에 없어야
    const cb = JSON.parse(fs.readFileSync(path.join(dir, '.pact/current_batch.json'), 'utf8'));
    for (const id of cb.task_ids) {
      assert.ok(!(id in r2.task_graph.tasks), `batch0 task ${id} 는 graph 에서 제외`);
    }
  } finally { cleanupProject(dir); }
});

test('DRV-1 prepare (--graph 없음) — already_prepared 재개는 task_graph 미출력(기존 출력 불변)', () => {
  const dir = makeProject();
  try {
    writeTasks(dir, [
      { id: 'NOGRAPH-001', allowed_paths: ['src/a.ts'] },
      { id: 'NOGRAPH-002', allowed_paths: ['src/b.ts'] },
    ]);
    runPact(['run-cycle', 'prepare', '--max=1'], dir); // 1차(--graph 없음)
    const r2 = JSON.parse(runPact(['run-cycle', 'prepare', '--max=1'], dir).stdout);
    assert.equal(r2.already_prepared, true);
    assert.equal(r2.task_graph, undefined, '재개도 --graph 없으면 task_graph 미출력(하위호환)');
  } finally { cleanupProject(dir); }
});
...
// ─── DOG-1: 사이클 내 stale rejected 화해 (reject→resume→merge 후 rejected 미잔존) ───
// 실측: 사이클 종료 merge-result 가 merged 에 든 task 를 rejected 에도 (중복 포함) 남겨,
// 축소 coordinator 가 SOT 만 읽고 머지 성공 task 를 Blocked 로 오기록. collect-one 은 사이클 내
// rejected/failures 를 append 만 하므로, appendMergeResult 가 settle(merged)된 task 를 제거하고
// 같은 task 반복 rejected 를 최신 1건으로 접어야 한다.
test('collect-one — reject→resume→merge 후 rejected 에 미잔존 + 중복 제거 (DOG-1 사이클 화해)', () => {
  const dir = makeProject();
  try {
    fs.writeFileSync(path.join(dir, '.gitignore'), '.pact/\n');
    sh('git add .gitignore && git commit -m gitignore', { cwd: dir });
    writeTasks(dir, [{ id: 'DOG-001', allowed_paths: ['src/a.ts'] }]);
    const prep = JSON.parse(runPact(['run-cycle', 'prepare', '--max=1'], dir).stdout);
    const tp = prep.task_prompts[0];

    // 1차: 워커 산출물 없음 → collect-one 이 rejected(status.json missing). current_batch 유지(같은 사이클).
    const c1 = JSON.parse(runPact(['run-cycle', 'collect-one', 'DOG-001'], dir).stdout);
    assert.deepEqual(c1.merged, []);
    assert.ok(c1.rejected.some(r => r.task_id === 'DOG-001'), '1차 rejected');

    // 2차: 같은 이유로 다시 rejected(중복 유발) — 화해 없으면 merge-result.rejected 에 2건 누적.
    runPact(['run-cycle', 'collect-one', 'DOG-001'], dir);
    const mrReject = JSON.parse(fs.readFileSync(path.join(dir, '.pact/merge-result.json'), 'utf8'));
    assert.equal(
      mrReject.rejected.filter(r => r.task_id === 'DOG-001').length, 1,
      `같은 task 반복 rejected 는 최신 1건만: ${JSON.stringify(mrReject.rejected)}`,
    );

    // 3차: 워커 재투입(부분작업 완성) → collect-one 이 머지 성공.
    simWorker(dir, tp, 'src/a.ts');
    const c3 = JSON.parse(runPact(['run-cycle', 'collect-one', 'DOG-001', '--commit-status'], dir).stdout);
    assert.deepEqual(c3.merged, ['DOG-001']);

    // 핵심: 머지된 task 는 누적 rejected/failures 에서 제거돼야 한다(축소 coordinator 오기록 방지).
    const mr = JSON.parse(fs.readFileSync(path.join(dir, '.pact/merge-result.json'), 'utf8'));
    assert.deepEqual(mr.merged, ['DOG-001']);
    assert.ok(
      !mr.rejected.some(r => r && r.task_id === 'DOG-001'),
      `머지 성공 task 가 rejected 에 잔존하면 안 됨: ${JSON.stringify(mr.rejected)}`,
    );
    assert.ok(
