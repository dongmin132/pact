...
```bash
node ${CLAUDE_PLUGIN_ROOT}/bin/pact run-cycle collect --commit-status
```

`--commit-status` 는 머지된 task 의 `tasks/*.md` 상태 스탬프(`status: done`)를 결정적으로 커밋한다(0토큰) — 헤드리스 드라이버(`pact drive`)와 대칭. 이게 없으면 tracked `tasks/*.md` 가 미커밋으로 남아 다음 `/pact:parallel` 의 prepare preflight(isClean)가 `uncommitted changes` 로 실패한다.
...
- 메인이 worktree에서 `git status` + 변경 파일 확인 후 다음 중 하나:
  - 산출물 있음 → 메인이 직접 status.json 작성 (`verify_results`는 실측 가능한 것만 채우고 나머지 `skip`), `pact merge` 재시도. **report.md 는 수기 작성하지 않는다** — merge/collect 시 `report-gen`이 status.json 에서 결정적 렌더한다.
...
### 3. report.md 관련 — 해당 없음 (report-gen 이 자동 렌더)

`report.md` 는 워커도 메인도 **수기 작성하지 않는다** — `pact merge`/`collect`가 게이트 **이전**에 `report-gen`으로 status.json 에서 결정적 렌더한다. status.json 만 있으면 report.md 는 항상 존재하고, status.json 이 없으면 **Fallback #1**(status.json 미작성)이 선점한다. 따라서 메인이 별도로 report.md 를 작성할 필요가 없다.
...
## 단계 7.5: bookkeeping 커밋 (다음 batch preflight 통과용)

`collect --commit-status` 는 `tasks/*.md` 상태 스탬프를 이미 커밋했다. 단계 7 이 갱신한 `PROGRESS.md`·`DECISIONS.md` 는 아직 tracked 미커밋 상태 — 다음 `/pact:parallel` 의 prepare preflight(isClean)가 이걸로 막히므로 여기서 커밋한다:

```bash
git add PROGRESS.md DECISIONS.md && git commit -m "pact: cycle bookkeeping" || true
```

`DECISIONS.md` 의 승인대기 항목은 단계 7에서 이미 '후보'로 표시돼 있어 이 커밋은 **기록 영속**일 뿐 결정 반영이 아니다(철학 5번 무위반). `|| true` 는 갱신이 없어 커밋할 게 없을 때 무해 통과용. (`.pact/` 는 ignore 이므로 merge-result·runs 등은 커밋 대상이 아니다.)
...
> 3. `/pact:parallel` 재호출 — prepare 가 멱등 재개한다 (진행 중 사이클은 `.pact/current_batch.json` 을 어댑트, 미완이면 다음 batch 를 자동 선택). 다음 batch 부터 컨텍스트 0 에서 시작.
>
> 충돌·blocked 처리만 남았으면 이 세션에서 마무리 후 /exit 해도 됨.
> (`/pact:resume <task_id>` 는 **회로차단된 단일 task 재시도 전용** — batch 재개용이 아니다.)
